...
阶段 6 本地 MVP 已覆盖实验数字链接、五条核心规则以及终端、版本化 JSON 和单文件离线 HTML。Git 证据链仍保留在后续路线图，不属于当前实现。
...
| `WRONG_BEST_MARK` | 阶段 6 已实现 | 按显式表格/列/方向配置检查最优与可选次优格式 |
| `UNFAIR_COMPARISON` | 阶段 6 已实现 | 报告用户声明受控配置字段的未允许差异风险 |
...
分类结果仍不是已确认链接；独立的阶段 5 身份服务随后生成 Claim ID/指纹。阶段 6 的表格规则只消费显式 table_checks 与 metric_directions，不从表头猜测方向或分组。
...
统一加载配置、一次论文扫描、Claim/链接、一次实验结果 catalog 和比较所需的最小实验配置字段，迁移身份并执行选定的五条规则，输出唯一、稳定排序的 `CheckResult`。当前不加载 Git 证据。

### 5.5 报告

`metricproof report --format html|json --output PATH` 基于与 `check` 相同的 `CheckResult` 生成报告，不维护第二套规则逻辑。HTML 是单文件、无脚本、无外部资源的离线文档；输出路径必须保持在项目根目录内，并采用原子替换。`--no-timestamp` 可生成字节稳定的结果。
...
- 指标别名和显式 `higher-is-better` / `lower-is-better` 方向。
- 表格 label、表头/数据行范围、label 列、metric 列和 best/second-best 格式。
- 全局或按指标的绝对容差、相对容差。
- 受控比较字段、允许差异和严重程度。
...
每个节点都可定位；缺失来源形成 link/input diagnostic，不能推断或伪造。
受控比较额外读取用户声明的本地 JSON/YAML `config_reference` 中的最小键集合，并保留 key、两侧 run/path/value 与允许差异理由。Git commit/worktree 证据链属于后续阶段，当前 `check` 不读取它们。
...
- Rule diagnostic：当前五条 MVP 规则的结果。
...
- 不可靠输入受控降级，不输出过度断言。
- 终端、JSON 与 HTML 共享同一 `CheckResult`。
- Windows、Linux、macOS 路径均基于 `pathlib`。
- 自动测试、静态检查、包构建和离线演示通过。
- HTML 保持单文件、离线、转义用户文本且无脚本；Git/GitHub evidence 未提前实现。
