...
链接候选必须由用户确认后才原子写入 `claims.yml`。check 实现 `STALE_VALUE`、`WRONG_DELTA`、`MISSING_PROVENANCE`、`WRONG_BEST_MARK` 和 `UNFAIR_COMPARISON`，详见 [linking-and-checking.md](linking-and-checking.md)。HTML 是同一 CheckResult 的单文件离线渲染，详见 [html-report.md](html-report.md)。`metricproof init`、Git evidence chain、GitHub Actions 与 SARIF 尚未实现；当前流程仍不依赖 AI、外部服务，也不执行 TeX 或用户代码。